...
}else{$.ajax({url:destinationUrl,timeout:settings.ajaxTimeout,success:function(data){var contentCacheable=data.match(/^(\s|<!--[\s\S]*?-->)*<\w+[^>]*\s+cacheable="(.*?)"/i)||data.match(/^(\s|<!--[\s\S]*?-->)*<\w+[^>]*\s+cacheable='(.*?)'/i);
...
var isMobileAgent=function(){return(navigator.userAgent.match(/iPhone/i)||navigator.userAgent.match(/iPad/i));
};
...
$.AmazonPopover={displayPopover:displayPopover,mouseTracker:mouseTracker,updateBacking:updateBacking};
...
amznJQ.available("jQuery",function(){amznJQ.available("search-js-autocomplete",function(){amznJQ.available("navbarInline",function(){window.navbar.initAutoFocus=function(){var $=jQuery,doc=document,first=false;
...
}function isAugmentor(key){return $.inArray(key,[0,16,20,17,18,224])>=0;
}var maybeControls=[8,32,37,38,39,40];
function isAlwaysControl(key){if($.inArray(key,maybeControls)>=0){return false;
}return(key<=46&&key!=9)||(key>90&&key<94)||(key>111&&key<146);
...
$(doc).keydown(function(e){if(canFocus()&&!isMaybeControl(e.keyCode)&&e.keyCode!=9&&!isAugmentor()&&!isAugmented(e)){iss.focus();
}});
};
...
amznJQ.available("jQuery",function(){(function(window,undefined){var $=window.jQuery,merchRE=/^me=/,refre=/(ref=[-\w]+)/,ddre=/_dd_/,ddaliasre=/(dd_[a-z]{3,4})(_|$)[\w]*/,deptre=/\{department\}/g,slashre=/\+/g,aliasre=/search-alias\s*=\s*([\w-]+)/,nodere=/node\s*=\s*([\d]+)/,dcs="#ddCrtSel",sdpc="searchDropdown_pop_conn",tostr=Object.prototype.toString,ddBox;
$.isArray=$.isArray||function(o){return tostr.call(o)==="[object Array]";
};
var SS=function(sb){var node,div;
...
node=$("<div id='srch_sggst'/>").css({left:p.left,top:p.top+d.height,width:d.width}).appendTo(sb.parent());
$(window).resize(move);
...
searchSuggest=new SS(searchBox);
lookup(opt,"form");
lookup(opt,"valInput");
lookup(opt,"dd");
lookup(opt,"submit");
...
}}function updateCrtSuggestion(){var val=keyword(),alias,categoryName,sg;
...
}else{var aliasName=ddBox.attr("value").match(aliasre);
...
}function combineSuggestions(crtSuggestions,extraData){var crtCategorySuggestions=extraData,combinedList=[],i=0,j=0,n=crtSuggestions.length,m,sg,cs;
if(extraData&&extraData.length){crtCategorySuggestions=extraData[0].nodes;
}crtCategorySuggestions=crtCategorySuggestions||[];
m=crtCategorySuggestions.length;
while(i+j<maxSuggestions&&i<n){sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i)};
combinedList.push(sg);
if(!i&&m>0){while(j<m&&j<maxCategorySuggestions){cs=crtCategorySuggestions[j];
sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i),alias:cs.alias,categoryName:cs.name};
combinedList.push(sg);
++j;
}}++i;
}curSize=combinedList.length;
return combinedList;
}function searchJSONSuggest(){suggestRequest&&suggestRequest.cleanup();
...
return;
}var str=encodeURIComponent(kw),suggestUrl=opt.protocol+"//"+opt.src+"?"+"method=completion"+"&q="+str+"&search-alias="+alias+"&client="+opt.cid+"&mkt="+opt.mkt+"&x=updateISSCompletion";
...
}return{suggest:bindSuggest,keypress:bindKeypress,submit:bindSubmit,blur:blur,keyword:keyword,searchAlias:searchAlias,searchNode:searchNode,stop:stop,start:start,encoding:encoding,focus:focus,offset:offset,keydown:keydown,initStaticSuggestions:initStatic,initDynamicSuggestions:initDynamic,updateAutoCompletion:updateCompletion,init:init};
};
