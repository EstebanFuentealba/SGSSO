// ------------------------------ omniture_rsid_aspnet.js ------------------------------
var s_account = "msstoaspnet";
if (window.location.hostname.toLowerCase().indexOf("weblogs.asp.net") > -1) {
    s_account = "msstoaspnetblogs";
}
var omniGuidPath = "://www.asp.net/omniture/analyticsid.aspx"; // update w/ path to aspx file or remove if js var is written out on each page
if (window.location.hostname.toLowerCase().indexOf("asp.net") == -1) {
...
var gAnalyticsIdStatus = (gAnalyticsId) ? "fpc" : "";
if (!gAnalyticsId) {
    try {
        gAnalyticsId = omni_guid;
        gAnalyticsIdStatus = (gAnalyticsId) ? "jsvar" : "";
    } catch (err) { };
}
if (omniGuidPath && !gAnalyticsId) {
    try {
        gAnalyticsIdStatus = "aspx";
...
// removed checkMeta,findsprops, findevars, findqueryparams, since not needed
...
// removed PageQuery(q), and queryString(key), omni_StripProtocol(), omni_StripQS(), findevents() since not needed
...
function s_doPlugins(s) {
    ...
    s.eVar45 = s.prop45 = gAnalyticsIdStatus;
...
// ------------------------------  omnihelper_aspnet.js  ------------------------------
function omniSetSiteVars() {
    s.linkInternalFilters = "javascript:,asp.net";
    ...
    s.prop1 = "asp.net";
    s.prop2 = "en-us";  // update to set programmatically if rolling out to other markets
    ...
    if (s.server.indexOf("www.asp.net") > -1) {
        s.channel = (temppatharr.length > 0) ? temppatharr[0] : "home";
        s.prop4 = (temppatharr.length > 1) ? s.channel + ": " + temppatharr[1] : "";
    } else {
        s.channel = s.server.replace(/\.asp\.net/, "");
        s.prop4 = (temppatharr.length > 0) ? s.channel + ": " + temppatharr[0] : "home";
    }

    // new variable for blog name, added 3/2/10
    if (s.server.indexOf("weblogs.asp.net") > -1) {
        s.prop5 = temppatharr[0];
    }
