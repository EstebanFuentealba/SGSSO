...
function MscomBeacon() {
    try {
        ...
        if (document.images) {
            ...
            imgArrayIndex++
        } else document.newasyncwrite('<IMG ALT="" BORDER="0" NAME="bImg" WIDTH="1" HEIGHT="1" SRC="' + srcString + '"/>');
        clickInfo = "";
        customInfo = ""
    } catch (e) {}
}
...
function MscomGetMUID() {
    if (window.varCustomerTracking != undefined && varCustomerTracking == 1) try {
        var muidsrc = window.location.protocol + "//analytics.microsoft.com/Sync.html";
        document.newasyncwrite('<iframe id="_msnFrame" src="' + muidsrc + '" style="z-index:-1;height:1px;width:1px;display:none;visibility:hidden;" onload="javascript:MscomBeacon();"></iframe>');
        fl = 1
    } catch (e) {
        fl = 0
    }
